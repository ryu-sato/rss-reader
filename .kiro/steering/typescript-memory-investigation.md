...
> **この文書の位置づけ**: 実施済みの調査記録であり、そこから導いた運用ルールは
> `.kiro/steering/testing.md`（Prisma デリゲートを `vi.mocked()` に通さない）にある。
> 日常の開発ではそちらだけ読めばよく、本書は「なぜそのルールがあるのか」を辿るための資料。
>
> 本文中のファイルパス（`src/lib/*` など）は調査当時のもので、その後のドメイン再編で
> 移動している。現在の配置は `.kiro/steering/structure.md` を参照。
