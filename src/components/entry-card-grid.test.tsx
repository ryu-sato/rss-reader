...
describe('EntryCardGrid — モーダル表示中の追加読み込みと次へナビゲーション', () => {
  ...
  it('【重複ページ】loadNavMore が既存の記事を含むページを返しても「次の記事」でまだ見ていない記事へ進める', async () => {
    // Arrange: モーダル表示中に新着記事が上位へ挿入され、offset ベースのページングが
    // ずれることで loadNavMore のページ2レスポンスに既存の entry-1 が再度含まれてしまう
    // ケースを再現する（IO は発火させず loadNavMore 専用パスを使う）。
    vi.stubGlobal('IntersectionObserver', NonIntersectingObserver)
    ...
    vi.stubGlobal(
      'fetch',
      stubFetch([{ entries: [entry1, entry2], hasNext: false }]),
    )
    ...
    // Act 1: entry-1 を開く → loadNavMore が発火し、entry-1 の重複を含むページ2を取得する
    fireEvent.click(screen.getByText('Article entry-1'))
    await waitFor(() => {
      expect(screen.getByTestId('article-modal')).toBeTruthy()
      expect(screen.getByRole('button', { name: '次の記事' })).not.toBeDisabled()
    ...
    // Act 2: 「次の記事」をクリックする。
    // navEntries に重複した entry-1 が残っていると findIndex が先頭の entry-1 に
    // 一致し続け、実際には entry-2 が読み込み済みなのに遷移できない不具合を検出する。
    fireEvent.click(screen.getByRole('button', { name: '次の記事' }))
    await waitFor(() => {
      expect(screen.getByTestId('article-modal').dataset.entryId).toBe('entry-2')
    })
...
describe('EntryCardGrid — モーダル表示中はフィルタを反映せず、閉じたときにまとめて反映する', () => {
  it('未読フィルタ表示中に記事を既読化しても、モーダルが開いている間は一覧から消えず、閉じると消える', async () => {
    // Arrange: meta: null（＝未読・あとで読む未登録）の記事を含む未読一覧。
    // モーダル内での自動既読化は entry-service 側で upsert されるだけで、
    // クライアント側にはこの entry:read イベントで isRead:true が伝わる。
    const entry1 = makeEntry('entry-1')
    const entry2 = makeEntry('entry-2')
    ...
    // Act 1: entry-1 を開く（モーダル内で自動既読化イベントが飛ぶ想定）
    fireEvent.click(screen.getByText('Article entry-1'))
    await waitFor(() => {
      expect(screen.getByTestId('article-modal')).toBeTruthy()
    })
    fireEvent(window, new CustomEvent('entry:read', { detail: { entryId: 'entry-1', feedId: 'feed-1' } }))

    // Assert 1: モーダル表示中は既読フィルタが反映されず、entry-1 は一覧に残ったまま
    expect(screen.getByText('Article entry-1')).toBeTruthy()

    // Act 2: モーダルを閉じる
    fireEvent.click(screen.getByRole('button', { name: '閉じる' }))
    await waitFor(() => {
      expect(screen.queryByTestId('article-modal')).toBeNull()
    })

    // Assert 2: 閉じた後は未読フィルタが再適用され、既読化された entry-1 は一覧から消える
    await waitFor(() => {
      expect(screen.queryByText('Article entry-1')).toBeNull()
    })
    expect(screen.getByText('Article entry-2')).toBeTruthy()
  })

  it('あとで読むフィルタ表示中に登録解除しても、モーダルが開いている間は一覧から消えず、閉じると消える', async () => {
    const entry1 = makeEntry('entry-1')
    entry1.meta = { id: 'meta-1', entryId: 'entry-1', isRead: false, isReadLater: true, createdAt: new Date(), updatedAt: new Date() }
    const entry2 = makeEntry('entry-2')
    entry2.meta = { id: 'meta-2', entryId: 'entry-2', isRead: false, isReadLater: true, createdAt: new Date(), updatedAt: new Date() }
    vi.stubGlobal('fetch', stubFetch([]))
    ...
    fireEvent(window, new CustomEvent('entry:updated', { detail: { entryId: 'entry-1', isReadLater: false } }))

    // モーダル表示中は即座に消えない（以前の実装ではここで消えてしまっていた）
    expect(screen.getByText('Article entry-1')).toBeTruthy()
